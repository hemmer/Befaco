...
struct NoisePlethora : Module {
	...
	size_t get_display_text(int led_id, std::span<char> text) override {
		...
		const NoisePlethora::Section section =
			led_id == NoisePlethora::SEGMENT_A ? NoisePlethora::SECTION_A : NoisePlethora::SECTION_B;
		text.front() = (section == SECTION_A ? textDisplayA : textDisplayB).load(std::memory_order_relaxed);
		return 1;
	}
};
